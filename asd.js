...
(function () {
    ...
    var ref = host + '/';
    var user_agent = Utils.baseUserAgent();
    var embed = 'https://hapi.hentaicdn.org/api/';
    var animelib_token = Lampa.Storage.get('animelib_token', '') + '';
    ...
    function addTokenToUrl(url) {
        if (animelib_token) {
            return Lampa.Utils.addUrlComponent(url, 'token=' + encodeURIComponent(animelib_token));
        }
        return url;
    }
    ...
    function getEpisodes(json) {
        var url = addTokenToUrl(embed + 'episodes');
        ...
        network.silent(component.proxyLink(url, prox, prox_enc), function (episodes) {
            if (episodes && episodes.data && episodes.data.length) {
                ...
                getPlayers(json.episodes[0], function () {
                    if (json.episodes[0].players && json.episodes[0].players.length) success(json);
                    else component.emptyForQuery(select_title);
                });
            } else component.emptyForQuery(select_title);
        }, function (a, c) {
    ...
    function getPlayers(episode, callback) {
        ...
        var url = addTokenToUrl(embed + 'episodes/' + episode.id);
        network.clear();
        network.timeout(1000 * 15);
        network.silent(component.proxyLink(url, prox, prox_enc), function (json) {
